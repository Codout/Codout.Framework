...
{
    public class DatabaseUtil : DatabaseCore
    {
        ...
        /// </summary>
        private readonly string _className = "Codout.Framework.NetStandard.DatabaseUtil.";
        ...
        /// </summary>
        /// <param name="objDbConnection">Objeto devidamente tipado da conexão</param>
        public DatabaseUtil(DbConnection objDbConnection)
        {
        ...
        public void Conect(string stringConexao)
        {
            try
            {
                ...
                Connection.Open();
                _comando = Connection.CreateCommand();
                Connected = true;
            }
            catch (Exception ex)
            {
        ...
        public void StartTransaction()
        {
            try
            {
                ...
                if (Connected)
                {
                    _transacao = Connection.BeginTransaction(IsolationLevel.ReadCommitted);
        ...
        {
            return ExecuteCommand(sql, true);
        }
        ...
        /// <returns>returna o número de linhas afetadas pelo commando SQL</returns>
        public int ExecuteCommand(string sql, bool upperCase)
        {
            if (!Connected)
            {
        ...
        public void PrepareCommand(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair)
        {
            PrepareCommand(sql, camposKeyValuePair, true);
        }
        ...
        public void PrepareCommand(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair, bool upperCase)
        {
        ...
        public void ExecuteCommandPrep(List<object> valores)
        {
            ExecuteCommandPrep(valores, true);
        }
        ...
        public void ExecuteCommandPrep(List<object> valores, bool upperCase)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <param name="valores">Valores para serem utilizados na execução do comando parametrizado</param>
        public void ExecuteCommandParametrized(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair, List<object> valores)
        {
            ExecuteCommandParametrized(sql, camposKeyValuePair, valores, true);
        }
        ...
        public void ExecuteCommandParametrized(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair, List<object> valores, bool upperCase)
        {
            ExecuteCommandParametrized(sql, camposKeyValuePair, valores, upperCase, false);
        }
        ...
        internal void ExecuteCommandParametrized(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair, List<object> valores, bool upperCase, bool isConsultaSql)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <param name="upperCase">se true, converte a query para maiúsculas</param>
        /// <returns>Retorna o resultado da contagem</returns>
        public int ExecuteCount(string sql, bool upperCase)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <param name="sql">Query a ser executada</param>
        /// <param name="upperCase">se true, converte a query para maiúsculas</param>
        public override void ExecuteQuery(string sql, bool upperCase)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <param name="camposKeyValuePair">Vetor de chave(nome campo)/valor(dbType do banco) de parametros da query, NÃO inserir '@' antes de cada nome de campo</param>
        /// <param name="valores">Valores para serem utilizados na execução do comando parametrizado</param>
        public void ExecuteQueryParametrized(string sql, List<KeyValuePair<string, DbType>> camposKeyValuePair, List<object> valores)
        {
            ExecuteCommandParametrized(sql, camposKeyValuePair, valores, true, true);
        }

        /// <summary>
        /// Libera o datareader caso esteja aberto
        /// </summary>
        internal void FreeDataReader()
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        public override object GetValue(int numero)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        public override byte[] GetValueBytes(string nomeColuna)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
            ...
            try
            {
                if (_readerAberto)
                {
                    return GetValueBytes(_reader.GetOrdinal(nomeColuna));
                }
                throw new Exception("O reader não está aberto, provavelmente não foi executado o método ExecutaConsulta");
            }
            catch (Exception ex)
        ...
        /// <returns>Retorna um vetor de bytes com o valor da coluna</returns>
        public override byte[] GetValueBytes(int numeroColuna)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <param name="nomeColuna">Nome da coluna</param>
        /// <returns>Retorna um objeto com o valor da coluna</returns>
        public override object GetValue(string nomeColuna)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        public Type GetFieldType(string nomeColuna)
        {
            if (!Connected)
            {
                throw new Exception("Comando não pode ser executado sem antes conectar ao banco, use DbCon.Conecta!");
        ...
        /// <summary>
        /// Obtem informações do schema do banco
        /// </summary>
        /// <returns>Schema do banco</returns>
        ...
        /// Obtem informações do schema do banco
        /// </summary>
        /// <param name="nomeColecao">Nome da coleção do banco</param>
        /// <returns>Schema do banco</returns>
        public DataTable GetSchema(string nomeColecao)
        {
            return Connection.GetSchema(nomeColecao);
        }
        #endregion
