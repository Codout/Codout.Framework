...
public static class QueryableExtensions
{
    ...
    {
        return queryable.ToDataSourceResult(request.Take, request.Skip, request.Sort, request.Filter, request.Aggregate,
            request.Group);
    }
        ...
        IEnumerable<Sort> sort, Filter filter, IEnumerable<Aggregator> aggregates, IEnumerable<Group> group)
    {
        ...
        if (groupSelectors?.Any() == true)
        {
            ...
            foreach (var source in groupSelectors.Reverse())
        ...
        if (take > 0) queryable = Page(queryable, take, skip);
        ...
        if (groupSelectors?.Any() == true)
            ...
            result.Groups = queryable.GroupByMany(groupSelectors);
        else
    ...
    internal static object Aggregates<T>(IQueryable<T> queryable, IEnumerable<Aggregator> aggregates)
    {
        ...
        if (aggregators?.Any() == true)
        {
            ...
            foreach (var group in groups)
            {
                ...
                foreach (var p in fieldProps.Keys) type.GetProperty(p.Name)?.SetValue(fieldObj, fieldProps[p], null);
                if (fieldObj != null) 
    ...
    private static IQueryable<T> Sort<T>(IQueryable<T> queryable, IEnumerable<Sort> sort)
    {
        ...
        if (enumerable?.Any() == true)
        {
            ...
            var ordering = string.Join(",", enumerable.Select(s => s.ToExpression()));
    ...
    private static Filter PreliminaryWork(Type type, Filter filter)
    {
        if (filter.Filters != null && filter.Logic != null)
        {
            ...
            foreach (var f in filter.Filters) newFilters.Add(PreliminaryWork(type, f));
        ...
        if (currentPropertyType == typeof(DateTime) && DateTime.TryParse(filter.Value.ToString(), out var dateTime))
        {
            ...
            filter.Value = new DateTime(localTime.Year, localTime.Month, localTime.Day, localTime.Hour,
    ...
    private static IEnumerable<Sort> GetDefaultSort(Type type, IEnumerable<Sort> sort)
    {
        if (sort == null)
        {
            ...
            //or contains id
                ...
                propertyInfo =
                    properties.FirstOrDefault(p => p.Name.IndexOf("id", StringComparison.OrdinalIgnoreCase) >= 0);
            //or just get the first property
            else
                propertyInfo = properties.FirstOrDefault();

            if (propertyInfo != null) sortByObject.Field = propertyInfo.Name;
