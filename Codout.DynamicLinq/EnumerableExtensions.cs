...
public static class EnumerableExtensions
{
    public static dynamic GroupByMany<TElement>(this IEnumerable<TElement> elements, IEnumerable<Group> groupSelectors)
    {
        ...
        var enumerable = groupSelectors as Group[] ?? groupSelectors.ToArray();

        var selectors = new List<GroupSelector<TElement>>(enumerable.Count());

        foreach (var selector in enumerable)
        {
        ...
        params GroupSelector<TElement>[] groupSelectors)
    {
        if (groupSelectors.Length > 0)
        {
            ...
            // Group by and return                
            return elements.GroupBy(selector.Selector).Select(g => new GroupResult
            {
                Value = g.Key,
