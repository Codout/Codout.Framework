...
public static class ImageExtensions
{
    ...
    public static void DrawRectangles(this SixLabors.ImageSharp.Image image, Brush brush, IEnumerable<Rectangle> toDraw, float? thickness = null)
        => image.Mutate(op =>
        {
    ...
    public static SixLabors.ImageSharp.Image DrawPoints(this SixLabors.ImageSharp.Image image, Brush brush, IEnumerable<Point> toDraw)
        => image.Clone(op =>
        {
    ...
    /// <returns>A copy of the given image with the points drawn onto.</returns>
    public static SixLabors.ImageSharp.Image DrawRectanglesAndPoints(this SixLabors.ImageSharp.Image image, Brush brush, IEnumerable<RectangleF> rectsToDraw, IEnumerable<PointF> pointsToDraw)
        => image.Clone(op =>
        {
            var delta = Math.Max(image.Width / 400, 1);
    ...
    public static SixLabors.ImageSharp.Image Extract(this SixLabors.ImageSharp.Image sourceImage, Rectangle sourceArea) => sourceImage.Clone(op => op.Crop(sourceArea));

    public static SixLabors.ImageSharp.Image Extract(this SixLabors.ImageSharp.Image sourceImage, Rectangle sourceArea, int extractedMaxEdgeSize, bool scaleUpToMaxEdgeSize = false)
