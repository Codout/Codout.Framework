...
public class StringEnumJsonConverter : JsonConverterFactory
{
...
public class JsonStringEnumConverter<TEnum> : JsonConverter<TEnum> where TEnum : struct, Enum
{
    ...
    public JsonStringEnumConverter()
    {
        ...
        foreach (TEnum value in Enum.GetValues(typeof(TEnum)))
        {
            ...
            if (attr?.Name != null)
            {
                _enumToString.TryAdd(value, attr.Name);
                _stringToEnum.TryAdd(attr.Name, value);
                _numberToEnum.TryAdd(num, value);
            }
            else
            {
                _enumToString.TryAdd(value, value.ToString());
                _stringToEnum.TryAdd(value.ToString(), value);
                _numberToEnum.TryAdd(num, value);
            }
        }
    }
