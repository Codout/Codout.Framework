...
namespace Codout.Framework.Common.Helpers
{
    public class LatLong
    {
        ...
        public string ToStringDMS(bool decorate)
        {
            ...
            s = decorate
                ? $"{sign}{Degrees.ToString().PadLeft(3)}° {Minutes.ToString().PadLeft(2)}' {DecimalSeconds.ToString(_round4).PadLeft(7)}\"" 
                : $"{sign}{Degrees.ToString().PadLeft(3)} {Minutes.ToString().PadLeft(2)} {DecimalSeconds.ToString(_round4).PadLeft(7)}";
        ...
        public string ToStringDM(bool decorate)
        {
            ...
            s = decorate 
                ? $"{sign}{Degrees.ToString().PadLeft(3)}° {DecimalMinutes.ToString(_round6).PadLeft(9)}'" 
                : $"{sign}{Degrees.ToString().PadLeft(3)} {DecimalMinutes.ToString(_round6).PadLeft(9)}";
            return s;
        }
        ...
        public string ToStringD(bool decorate)
        {
            ...
            s = decorate 
                ? $"{sign}{DecimalDegrees.ToString(_round8).PadLeft(12)}°" 
                : $"{sign}{DecimalDegrees.ToString(_round8).PadLeft(12)}";
        ...
        private int MinuteOrSecondIs60(double minuteOrSec, string roundStr)
        {
            try
            {
                var minorsecstr = minuteOrSec.ToString(roundStr);

                if (Convert.ToDouble(minorsecstr) >= 60 - EpsilonDeg && Convert.ToDouble(minorsecstr) <= 60 + EpsilonDeg)
                    return 1;
